...
namespace OX.DataBinding
{
    public class Bindable
    {
        ...
        public static T Source<T>(T instance)
        {
            ...
            {
                BindingPairHolder.DisableBindingDeclaration();
                T t = (T)Activator.CreateInstance(existingDynamicType, instance);
                BindingPairHolder.EnableBindingDeclaration();
                return t;
            }
            ...
            foreach (PropertyInfo prop in typeToBind.GetProperties())
            {
                ...
                if (prop.CanRead)
                {
                    ...
                    // Write to console
                    ilGetter.Emit(OpCodes.Ldstr, string.Format("Inside {0}.{1}...", typeToBind.Name, prop.Name));
                    ilGetter.Emit(OpCodes.Call, typeof(Console).GetMethod("WriteLine", new Type[] { typeof(string) }));
